...
| 7 | Endpoint `/sse/mj` dans `facade/server.py` + migration `activity/*` en `mcp_role="mj"` | Façade MJ opérationnelle, tools MJ invisibles côté joueurs | #6 | `DONE` (2026-04-20, 72 TU verts, isolation player/mj confirmée) |
| 8 | `AutonomousMJ` + commande `mj/autorun` + boucle tool-use + safety guard | Port Beholder fonctionnel sur action/vérité | #3, #4, #5, #7 | `DONE` (run réel 2026-04-20 louis/isabelle round_robin, 8 iter en 3min23, end naturel par le MJ, 85 TU verts) |
